  ...
  .action(async (kind: string, pattern: string) => {
    ...
    try {
      ...
      if (kind === 'github') {
        ...
        console.log(
          `note: add a bot token with \`think serve creds add slack ${pattern}\` (reads from stdin or $THINK_SLACK_PAT). See docs/serve.md for bot setup (required scopes: channels:history, groups:history, channels:read, groups:read, reactions:read; add files:read, canvases:read, users:read to capture huddle transcripts).`,
        );
        console.log(
