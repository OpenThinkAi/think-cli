...
import { insertEngram, pruneExpiredEngrams } from '../../src/db/engram-queries.js';
...
// AGT-1302: pruning used to delete every expired row, evaluated or not — which
// is how think-cli#95 lost 20 unread decision logs to a single `think curate`.
// The guard makes "nothing prunes an unmigrated row" a property of the DELETE
// rather than of the call order, so no future caller can run ahead of
// migrateStrandedEngrams and destroy what it was about to rescue.
describe('pruneExpiredEngrams — never deletes an unmigrated row (AGT-1302)', () => {
  let originalHome: string | undefined;
  let tmpHome: string;
  const cortex = 'engram-prune-test';
  const PAST = '2026-05-01T00:00:00.000Z';

  beforeEach(() => {
    originalHome = process.env.THINK_HOME;
    tmpHome = mkdtempSync(join(tmpdir(), 'think-engram-prune-'));
    process.env.THINK_HOME = tmpHome;
    closeAllCortexDbs();
    getCortexDb(cortex).prepare(
      `INSERT INTO engrams (id, content, created_at, expires_at, evaluated_at)
       VALUES ('pending', 'expired but never evaluated', ?, ?, NULL),
              ('done', 'expired and already evaluated', ?, ?, ?)`,
  ...
  it('prunes an expired evaluated row and leaves an expired unevaluated one', () => {
    expect(pruneExpiredEngrams(cortex)).toBe(1);
    const remaining = getCortexDb(cortex)
      .prepare('SELECT id FROM engrams ORDER BY id')
