...
// Returns a Float32Array unit vector with 1.0 at `pos`, zeros elsewhere.
...
// Returns a Uint8Array BLOB for the given Float32Array.
...
describe('searchVectors (AGT-275)', () => {
  ...
  beforeEach(() => {
    ...
    for (const { content, vec } of FIXTURES) {
      ...
      // Manually patch the embedding column since insertMemory doesn't
      // accept it yet (embedding pipeline is a separate ticket).
  ...
  it('brute-force vs sqlite-vec: same top-1 id and similarity for x-axis query', () => {
    // Brute-force baseline.
    const cfg = getConfig();
    cfg.search = { engine: 'brute-force' };
    saveConfig(cfg);
    const bfResults = searchVectors(cortex, axis(0), 1);
    expect(bfResults[0].similarity).toBeCloseTo(1.0, 2);
    ...
    // In both cases similarity should be ≈ 1.0 (cosine scale) and the id must
    // match.
    ...
    expect(svResults[0].similarity).toBeCloseTo(1.0, 2);
  });
});
