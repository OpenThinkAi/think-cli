/**
 * Unit tests for checkRepoIndex() — AGT-1308 AC1/AC5.
 *
...
describe('checkRepoIndex (AGT-1308)', () => {
  ...
  it('fails, fixable, for the pure stale state (nothing to restore)', () => {
    const result = checkRepoIndex({ repoPath, plan: () => [] });

    expect(result.status).toBe('fail');
    expect(result.fixable).toBe(true);
    expect(result.detail).toContain('stale behind a plumbing-advanced HEAD');
    expect(result.detail).toContain('No in-flight worktree changes');
  });

  it('says how many genuine appends a repair would preserve', () => {
    const result = checkRepoIndex({
      repoPath,
      plan: () => [
    ...
    expect(result.status).toBe('fail');
    expect(result.detail).toContain('2 in-flight worktree appends would be preserved');
  });
