...
## Third-party content data flow (redaction is currently dead code)

The proxy server stores connector event payloads opaquely (`payload_json` in the `events` table) — it does not parse, validate, or redact them. Redaction was designed to run on the **CLI side**, during `think subscribe poll`, before a payload landed as local engram content (AGT-066). Two layers, in order:

1. **Baseline PII strip** — recursive walk that drops any field whose key matches the baseline patterns (`email`, `*_email`, `email_*`, `gpg`, `gpg_*`, `*_gpg`, `ip`, `*_ip`, `ip_*`, `x-real-ip`, `x-forwarded-for`, `client-ip`, `phone`, `*_phone`, `phone_*`). Case-insensitive on the standard hyphenated form for headers. Each PII category uses three suffix shapes (bare, `_x` suffix, `x_` prefix) — symmetry matters because the prefix shape (e.g. `phone_number`) is often the canonical field name in real payloads.

2. **Per-subscription redact selectors** — JSONPath-subset (`$.a.b.c` form only, no array indices/wildcards/filters/recursive descent). Configured via `think subscribe redact-set <id> <selector...>`; stored at `subscriptions.redact[<id>]` in `~/.config/think/config.json`.

**Neither layer runs today.** `3.0.0` removed the local engram write path (and `--legacy-engrams`) that these two functions (`stripBaselinePii`, `applyRedactSelectors`) used to feed into — that ingestion path had in fact been dead since proxy-curated events (AGT-389) replaced local polling as the real pipeline, well before `3.0.0` formalized it. `think subscribe poll` is now a no-op; the proxy's terminal-event curator produces the curated memory directly from the event payload and neither redaction layer, nor `validateEngramContent`'s length-cap/prompt-injection scan, sits in that path. `think subscribe redact-set` and `think subscribe show` still work as configuration surfaces — they just don't affect anything downstream. See [`docs/serve.md`](docs/serve.md#third-party-content-data-flow--redact-agt-066) for the full detail.

**Caveat: there is currently no redaction or content-length filtering anywhere in the connector pipeline.** If you operate a `think serve` proxy against a connector source you don't fully trust, treat every row that lands on the team cortex as unfiltered third-party content until a future ticket either wires a redaction layer into the terminal-event curator (server-side, where the content actually flows today) or removes the dead CLI-side surface. Filing a follow-up ticket rather than assuming this is handled.

## Scope of this document

This file covers the server's source-credential surface and the CLI-side redact layer that used to depend on it. Other server-side concerns (auth token comparison, SQLite file permissions, `/v1/cortexes/*` retirement semantics) are documented in code comments and [`packages/cli/docs/serve.md`](docs/serve.md). CLI threats (cortex sync, untrusted peer memories, config tampering) live at the root [`SECURITY.md`](../../SECURITY.md).
