...
- **`think retro` accepts `--silent`, like `think sync` and `think event`.** It rejected the flag with `unknown option '--silent'`, so an agent that appends `--silent` to all three write commands lost the lesson whenever a retro was chained after a sync with `&&`. `--silent` suppresses normal stdout output on both the daemon path and the daemon-down path; the daemon-unreachable note still goes to stderr, and errors (a daemon refusal, the quality gate) are still reported. (#98)
...
**Opt-in.** With no `cortex.llm` block, every operation runs on Anthropic exactly as it did in 2.5.2, and an existing `cortex.local` block keeps the scope it always had (curation). Nothing moves to a different model unless you name it.
...
- **`cortex.llm` — a provider registry with per-operation routing.** Declare any number of named providers (`kind: "openai"` for anything speaking `/chat/completions`, or `kind: "anthropic"`), pick a `default`, and assign individual operations via `operations`. Assignable: `curation`, `event-detection`, `episode`, `terminal-event`, `retro-dedupe`, `summary`, `dashboard`, `long-term`, `compaction`, `supersession`. Note that `dashboard` covers the status digest — the panel summaries `think dashboard` renders — and not the dashboard's interactive `ask`, which is a separate, non-assignable operation (see "Not changed"). `fallback` names where to go when a task exceeds a provider's `ctxBudget`; a fallback that leaves the machine still requires consent, and if the fallback cannot take the task either the work is skipped and left pending rather than retried elsewhere. `apiKeyEnv` reads the key from an environment variable rather than storing it in a config file that syncs. `offMachine` declares whether a provider puts content on the network and therefore whether consent is required; it is inferred from the endpoint when omitted, and setting it to `false` suppresses the consent requirement for a non-loopback host you trust (a box on your own LAN, or a hostname alias that resolves to loopback — inference is by hostname, so `my-laptop.local` is treated as off-machine until you say otherwise). Unknown keys in `operations` now warn instead of being silently ignored.
...
- **`disableThinking`** sends `chat_template_kwargs: { enable_thinking: false }`, so reasoning models don't spend a structured-output budget on a preamble the caller discards. Servers that don't implement it ignore the field silently — think cannot tell whether it took effect, so verify against your server if it matters. The same is true of `response_format: json_schema`, which `mlx_lm.server` accepts and ignores; the README carries a one-line curl to check yours before routing the strict-schema operations (`compaction`, `supersession`) to it.
...
- **An existing `cortex.local` / `llmProvider` config keeps its original scope: curation only.** When those settings were written, curation was the only operation that consulted the router, so that is the whole of what they opted into — and an upgrade does not extend them to cover nine operations they never named. `summary`, `dashboard`, `long-term`, `episode`, `terminal-event`, `retro-dedupe`, `compaction` and `supersession` continue to run on Anthropic for a legacy config. Routing them requires `cortex.llm`, which is new surface and therefore a deliberate choice.
...
- **The dashboard's `ask` stays on the Claude Agent SDK** and is not assignable via `cortex.llm.operations` — distinct from the `dashboard` key above, which routes the status digest. There is no operation name for `ask`; setting one has no effect. It is agentic rather than one-shot — a multi-turn loop calling MCP tools — and the one-shot client contract cannot express it without either dropping its tools or carrying MCP servers, turn caps and tool allowlists no other caller needs.
...
- **Structural evidence gate on supersession.** Curation retrieves supersession candidates by embedding similarity — shared vocabulary — but hiding a record demands evidence of a shared *subject*. An LLM-proposed supersession is now accepted only when the two entries share a topic tag or a named entity (repo slug, ticket id, `#N` ref, filename, hyphenated identifier). When both entries carry structure and none of it overlaps, the link is rejected and logged to the daemon log (`<think home>/daemon.log`, lines beginning `supersession evidence gate:` / `[supersession] evidence gate:`) so a blocked link is auditable; plain structure-free prose (a "use pnpm" retro superseding "use npm") still defers to the LLM as before. A pairwise cosine floor (`compaction.supersedeMinCosine`, default 0.4, applied to both compaction and retro supersession) backstops the gate. The gate's test fixtures are the real mislinked and correctly-consolidated pairs from the issue report.
- **`think supersession` — subcommands `list`, `show <id>`, `revert <id>`.** Superseded entries were previously discoverable only by querying SQLite directly. `list` enumerates hidden entries (most recent first, with totals), `show` explains an entry's supersession state in both directions (what hid it, what it hid), and `revert` restores an entry to active recall. Supersession state is per-peer local index state, so a revert is complete on that machine. (Noun-anchored group, same management-surface pattern as `think cortex`, `think daemon`, `think mcp`.)
- **Recall marks superseded entries.** Entries returned via `--include-superseded`/`--full` now carry `superseded_by`/`superseded_at` in JSON output and a `superseded_by="<id>"` attribute on the `<recall-result>` envelope, so a hidden record is distinguishable from an absent one. The fields are optional on the `RecallEntry` type (always set by this package's producers), so consumers that construct `RecallEntry` values are unaffected — the addition is non-breaking.
