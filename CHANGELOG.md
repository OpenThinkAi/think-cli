...
- **`disableThinking`** sends `chat_template_kwargs: { enable_thinking: false }`, so reasoning models don't spend a structured-output budget on a preamble the caller discards. Servers that don't implement it ignore the field silently — think cannot tell whether it took effect, so verify against your server if it matters.
