...
- **`cortex.repo` is security-sensitive configuration.** `think cortex setup` validates the URL shape on input, but if you edit `~/.config/think/config.json` by hand (or follow a tutorial that tells you to), a malformed URL can give an attacker code execution the next time you run a cortex-syncing command. Accepted prefixes: `https://` (preferred), `ssh://`, `git://`, `git@<host>:<path>` (ssh shortcut — `<host>` is any hostname, not the literal word "host"), and `http://` (permitted but not recommended — traffic is unencrypted).
