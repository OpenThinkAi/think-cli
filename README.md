...
- **`cortex.repo` is security-sensitive configuration.** `think cortex setup` validates the URL shape on input, but if you edit `~/.config/think/config.json` by hand (or follow a tutorial that tells you to), a malformed URL can give an attacker code execution the next time you run a cortex-syncing command. Accepted transports: `https://` (preferred), `ssh://`, `git://`, `git@<HOST>:<PATH>` (ssh shortcut; `<HOST>` is any hostname), and `http://` (accepted for completeness but **not recommended** — traffic is unencrypted, subject to MITM on any untrusted network).
