...
| **Connector sources** (GitHub, Linear, Notion, Slack, meeting transcripts — via a `think serve` instance) | The proxy's per-subscription poll scheduler, running on whatever host operates `think serve` — not the local `think` CLI | Read requests against each connector's API using the stored credential; the returned event payload — for a Slack huddle or a meeting, the verbatim spoken transcript with speaker names — is stored and curated server-side, then published to the team cortex, which reaches this CLI as an ordinary `cortex pull`. | See [`SECURITY-serve.md`](packages/cli/SECURITY-serve.md) for the proxy's own threat model (credential-at-rest, connector egress). |
...
| Proxy terminal-event curation (`think serve`) | Automatic, per settled connector event | One connector event's payload (GitHub/Linear/Notion/Slack/meeting-transcript), wrapped in `<data>` tags, sent to produce the memory published to the team cortex. |
